...
export default class Game extends Phaser.Scene {
    ...
    create() {
        ...
        const body = this.mouse.body as Physics.Arcade.Body
    ...
    private wrapBookcases() {
        ...
        if(this.bookcase1.x + width < scrollX) {
            ...
            const overlap = this.windows.find(window => {
                return Math.abs(this.bookcase1.x - window.x) <= this.bookcase1.width
            })
        ...
        if(this.bookcase2.x + width < scrollX) {
            ...
            const overlap = this.windows.find(window => {
                return Math.abs(this.bookcase2.x - window.x) <= this.bookcase2.width
            })
    ...
    private handleOverlapLaser(
        ...
        const mouse = (obj1 == this.mouse ? obj1: obj2) as RocketMouse
        mouse.kill()
    }
}